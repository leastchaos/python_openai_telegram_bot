...
def reset(update: Update, context: CallbackContext) -> None:
    ...
    reply_query(update, 'Your conversation history has been reset.')


def start(update: Update, context: CallbackContext) -> None:
    # Create an inline keyboard with buttons for the /reset and /temperature commands
    reply_text(update, 'Hi! I am a chatbot powered by ChatGPT. You can talk to me about anything you like.')
...
def get_reply_markup():
    ...
    keyboard = [[InlineKeyboardButton('Reset history', callback_data='reset'),
...
def chat(update: Update, context: CallbackContext):
    ...
    response = openai.Completion.create(
        ...
        prompt=prompt,
        max_tokens=1024,
        temperature=user_temperature.get(update.effective_user.id, 0.5),
    )
    result = response.choices[0].text
    if not result:
        print("empty result")
        reply_text(update, "please try again.")
        return
